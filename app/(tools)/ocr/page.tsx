    ...
    <main className="min-h-screen bg-gray-50">
      <div className="max-w-2xl mx-auto px-4 py-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-5">
          {!isDone && (
            <>
              <DropZone onFilesAdded={(f) => addFiles([f[0]])} multiple={false}
                label="Upload a scanned or image-based PDF" hint="Single PDF file · Max 100MB" />
              {hasFiles && (
                <>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {languages.map((lang) => (
                        <button key={lang.value} onClick={() => setLanguage(lang.value)}
                          className={`py-2 rounded-xl border text-sm font-medium transition-all ${language === lang.value ? "border-cyan-400 bg-cyan-50 text-cyan-700" : "border-gray-200 text-gray-600 hover:border-gray-300"}`}>
                          {lang.label}
                        </button>
                      ))}
                    </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {([{ value: "text", label: "📄 Extract Text" }, { value: "searchable-pdf", label: "🔍 Searchable PDF" }] as const).map(({ value, label }) => (
                        <button key={value} onClick={() => setOutputMode(value)}
                          className={`py-2.5 rounded-xl border text-sm font-medium transition-all ${outputMode === value ? "border-cyan-400 bg-cyan-50 text-cyan-700" : "border-gray-200 text-gray-600 hover:border-gray-300"}`}>
                          {label}
                        </button>
                      ))}
                    </div>
                  ...
                  <div className="flex items-start gap-2.5 bg-blue-50 border border-blue-100 rounded-xl p-3">
                    ...
                    <p className="text-xs text-blue-600 leading-relaxed">
                      Text-based PDFs are processed instantly in browser. Scanned image PDFs require server-side OCR.
                    </p>
                  </div>
                </>
              )}
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <p className="text-sm font-semibold text-gray-700 flex items-center gap-2">
                  <ScanText className="w-4 h-4 text-cyan-500" /> Extracted Text
                </p>
                <button onClick={handleCopy} className="flex items-center gap-1.5 text-xs font-medium text-gray-500 hover:text-cyan-600 transition-colors">
                  {copied ? <CheckCheck className="w-3.5 h-3.5 text-green-500" /> : <Copy className="w-3.5 h-3.5" />}
                  {copied ? "Copied!" : "Copy all"}
                </button>
              </div>
              <textarea readOnly value={extractedText} rows={12}
