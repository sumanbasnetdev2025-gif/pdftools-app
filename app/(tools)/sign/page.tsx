...
import {
  ...
  Trash2, ChevronLeft, ChevronRight, ZoomIn, ZoomOut, X,
  Check, RotateCw, Users, Stamp, Save, Plus,
} from "lucide-react";
...
const SIGNER_COLORS = ["#F59E0B", "#3B82F6", "#10B981", "#EF4444", "#8B5CF6", "#EC4899"];
const TYPED_FONTS = [
...
export default function SignPage() {
  ...
  const [rotateStart, setRotateStart] = useState({ angle: 0, mouseAngle: 0 });
  ...
  const saveSigToStorage = (dataUrl: string, label: string) => {
    ...
    try { localStorage.setItem(SAVED_SIG_KEY, JSON.stringify(updated)); } catch {}
  };
  ...
  const handlePageClick = (e: React.MouseEvent<HTMLDivElement>) => {
    if (!currentSigDataUrl || draggingId || resizingId || rotatingId) return;
    ...
    const newSig: PlacedSig = {
      ...
      wPct: sigType === "stamp" ? 18 : 22,
      hPct: sigType === "stamp" ? 18 : 10,
      rotation: 0,
      page: currentPage,
  ...
  const onSigMouseDown = (e: React.MouseEvent, id: string) => {
    ...
    setDragOffset({
      x: e.clientX - rect.left - (sig.xPct / 100) * rect.width,
      y: e.clientY - rect.top - (sig.yPct / 100) * rect.height,
    });
    setDraggingId(id);
  };
  ...
  const onPageMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const rect = container.getBoundingClientRect();
    if (draggingId) {
      setPlacedSigs(prev => prev.map(s => s.id !== draggingId ? s : {
        ...s,
        xPct: Math.max(0, Math.min(100 - s.wPct, ((e.clientX - rect.left - dragOffset.x) / rect.width) * 100)),
        yPct: Math.max(0, Math.min(100 - s.hPct, ((e.clientY - rect.top - dragOffset.y) / rect.height) * 100)),
      }));
    }
    if (resizingId) {
      ...
      setPlacedSigs(prev => prev.map(s => s.id !== resizingId ? s : {
        ...s,
        wPct: Math.max(5, ((e.clientX - rect.left - sigX) / rect.width) * 100),
        hPct: Math.max(3, ((e.clientY - rect.top - sigY) / rect.height) * 100),
      }));
    }
    if (rotatingId) {
      const sig = placedSigs.find(s => s.id === rotatingId)!;
      ...
      const delta = mouseAngle - rotateStart.mouseAngle;
      setPlacedSigs(prev => prev.map(s => s.id !== rotatingId ? s : {
        ...s, rotation: rotateStart.angle + delta,
      }));
    }
  };

  const onPageMouseUp = () => { setDraggingId(null); setResizingId(null); setRotatingId(null); };
  ...
  const onRotateMouseDown = (e: React.MouseEvent, id: string) => {
    ...
    const sigCy = ((sig.yPct + sig.hPct / 2) / 100) * rect.height + rect.top;
    const mouseAngle = Math.atan2(e.clientY - sigCy, e.clientX - sigCx) * (180 / Math.PI);
    setRotateStart({ angle: sig.rotation, mouseAngle });
    setRotatingId(id);
    setActiveId(id);
  };
  ...
  const addSigner = () => {
    ...
    const id = `s${Date.now()}`;
    setSigners(prev => [...prev, { id, name: newSignerName.trim(), color: SIGNER_COLORS[prev.length % SIGNER_COLORS.length] }]);
    setActiveSigner(id);
    setNewSignerName("");
  ...
  const exportPDF = async () => {
    ...
    try {
      ...
      for (const sig of placedSigs) {
        ...
        page.drawImage(embed, { x, y, width: w, height: h, opacity: 0.95, rotate: degrees(sig.rotation) });
      }
      const out = await doc.save();
      const blob = new Blob([out], { type: "application/pdf" });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url; a.download = pdfFile.name.replace(".pdf", "_signed.pdf");
      a.click(); URL.revokeObjectURL(url);
    } finally { setProcessing(false); }
  };

  const currentPageSigs = placedSigs.filter(s => s.page === currentPage);
  const activeSig = placedSigs.find(s => s.id === activeId);
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .saved-del { position: absolute; top: 3px; right: 3px; width: 18px; height: 18px; background: #ef4444; border-radius: 50%; display: flex; align-items: center; justify-content: center; cursor: pointer; opacity: 0; transition: opacity 0.15s; }
        ...
        .placed-del { width: 24px; height: 24px; border-radius: 6px; background: transparent; border: none; color: #444; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.15s; }
        ...
        /* Page wrap */
        .page-wrap { position: relative; box-shadow: 0 8px 48px rgba(0,0,0,0.7); border-radius: 4px; overflow: hidden; user-select: none; flex-shrink: 0; }
        .page-wrap img { display: block; width: 100%; pointer-events: none; }

        /* Sig overlay */
        .sig-ov { position: absolute; cursor: move; border: 2px solid transparent; border-radius: 4px; transition: border-color 0.15s; transform-origin: center; }
        .sig-ov:hover { border-color: rgba(245,158,11,0.5); }
        .sig-ov.active-ov { border-color: #F59E0B; }
        .sig-ov img { width: 100%; height: 100%; object-fit: contain; display: block; pointer-events: none; }
        .sig-del-btn { position: absolute; top: -10px; right: -10px; width: 20px; height: 20px; background: #ef4444; border-radius: 50%; display: flex; align-items: center; justify-content: center; cursor: pointer; border: 2px solid #0a0a0a; color: white; }
        .sig-resize { position: absolute; bottom: -6px; right: -6px; width: 14px; height: 14px; background: #F59E0B; border-radius: 3px; cursor: se-resize; border: 2px solid #0a0a0a; }
        .sig-rotate { position: absolute; top: -24px; left: 50%; transform: translateX(-50%); width: 18px; height: 18px; background: #3B82F6; border-radius: 50%; display: flex; align-items: center; justify-content: center; cursor: grab; border: 2px solid #0a0a0a; color: white; }
        .sig-rotate:active { cursor: grabbing; }
        .sig-signer-badge { position: absolute; bottom: -20px; left: 0; font-size: 0.62rem; font-weight: 700; white-space: nowrap; padding: 2px 6px; border-radius: 4px; }
      ...
      <div className="sign-root">
        ...
        <div className="sign-bar">
          <span className="sign-bar-title">PDF<span>Master</span> — Sign</span>
          <div className="sign-bar-right">
            {pdfFile && (
              <button className="btn btn-ghost btn-sm" onClick={() => { setPdfFile(null); setPageImages([]); setPlacedSigs([]); setStep("upload"); setCurrentSigDataUrl(null); }}>
                <RotateCcw size={12} /> New
              </button>
            )}
            <button className="btn btn-amber" disabled={placedSigs.length === 0 || processing} onClick={exportPDF}>
              <Download size={13} />
        ...
        {step === "upload" && (
          <div className="upload-zone">
            ...
            <input ref={fileInputRef} type="file" accept=".pdf" hidden onChange={e => { const f = e.target.files?.[0]; if (f) handleFile(f); }} />
          </div>
        )}
          ...
          <div className="sign-layout">
              ...
              <div className="panel-tabs">
                {(["create", "saved", "placed"] as const).map(t => (
                  <button key={t} className={`panel-tab${tab === t ? " active" : ""}`} onClick={() => setTab(t)}>
                    {t === "create" ? "Create" : t === "saved" ? "Saved" : `Placed (${placedSigs.length})`}
                  </button>
                ))}
              </div>
              ...
              <div className="panel-scroll">
                ...
                {tab === "create" && (
                  <>
                    {/* Signers */}
                    <div>
                      <p className="panel-lbl" style={{ marginBottom: "10px" }}>Signers</p>
                      <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                        {signers.map(s => (
                          <div key={s.id} className={`signer-chip${activeSigner === s.id ? " active" : ""}`} onClick={() => setActiveSigner(s.id)}>
                            <span className="signer-dot" style={{ background: s.color }} />
                            <span className="signer-name">{s.name}</span>
                            {activeSigner === s.id && <Check size={12} style={{ color: "#F59E0B" }} />}
                          </div>
                        ))}
                        {showAddSigner ? (
                          <div style={{ display: "flex", gap: "6px" }}>
                            <input className="text-input" style={{ flex: 1, padding: "7px 10px", fontSize: "0.82rem" }}
                              placeholder="Name…" value={newSignerName}
                              onChange={e => setNewSignerName(e.target.value)}
                              onKeyDown={e => e.key === "Enter" && addSigner()} autoFocus />
                            <button className="btn btn-amber btn-sm" onClick={addSigner}><Check size={12} /></button>
                            <button className="btn btn-ghost btn-sm" onClick={() => setShowAddSigner(false)}><X size={12} /></button>
                          </div>
                        ) : (
                          <button className="btn btn-ghost btn-sm" style={{ justifyContent: "center" }} onClick={() => setShowAddSigner(true)}>
                    ...
                    {/* Sig type */}
                    <div>
                      <p className="panel-lbl" style={{ marginBottom: "10px" }}>Signature Type</p>
                      <div className="sig-types">
                        {([
                          { t: "draw", icon: <Pen size={14} />, lbl: "Draw" },
                          { t: "type", icon: <Type size={14} />, lbl: "Type" },
                          { t: "image", icon: <ImageIcon size={14} />, lbl: "Image" },
                          { t: "stamp", icon: <Stamp size={14} />, lbl: "Stamp" },
                        ] as { t: SigType; icon: React.ReactNode; lbl: string }[]).map(({ t, icon, lbl }) => (
                          <button key={t} className={`sig-type-btn${sigType === t ? " active" : ""}`}
                            onClick={() => { setSigType(t); setCurrentSigDataUrl(null); }}>
                            {icon}<span>{lbl}</span>
                          </button>
                        ))}
                      </div>
                    ...
                    {sigType === "draw" && (
                      <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                        <div className="draw-wrap">
                          <canvas ref={drawCanvasRef} width={520} height={200}
                            onMouseDown={startDraw} onMouseMove={draw} onMouseUp={endDraw} onMouseLeave={endDraw}
                            onTouchStart={startDraw} onTouchMove={draw} onTouchEnd={endDraw} />
                          <div className="draw-hint">Sign here</div>
                        </div>
                        <div style={{ display: "flex", gap: "6px" }}>
                          <button className="btn btn-ghost" style={{ flex: 1 }} onClick={clearCanvas}><Trash2 size={12} /> Clear</button>
                          <button className="btn btn-amber" style={{ flex: 1 }} onClick={confirmDraw}><Check size={12} /> Use</button>
                        </div>
                      </div>
                    )}
                    ...
                    {sigType === "type" && (
                      <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                        <input className="text-input" placeholder="Your name…" value={typedText} onChange={e => setTypedText(e.target.value)} />
                        <div className="font-grid">
                          {TYPED_FONTS.map((f, i) => (
                            <button key={f.label} className={`font-btn${typedFont === i ? " active" : ""}`} onClick={() => setTypedFont(i)}>{f.label}</button>
                          ))}
                        </div>
                        {typedText && (
                          <div className="sig-preview">
                            <span style={{ font: TYPED_FONTS[typedFont].css, color: "#1a1a2e" }}>{typedText}</span>
                          </div>
                        )}
                        <button className="btn btn-amber" onClick={generateTyped} disabled={!typedText.trim()}><Check size={12} /> Use</button>
                      </div>
                    )}
                    ...
                    {(sigType === "image" || sigType === "stamp") && (
                      <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                        <button className="btn btn-ghost" style={{ justifyContent: "center", padding: "18px" }}
                          onClick={() => (sigType === "stamp" ? stampInputRef : imgInputRef).current?.click()}>
                          <Upload size={14} /> {sigType === "stamp" ? "Upload stamp / seal" : "Upload signature image"}
                        </button>
                        <input ref={imgInputRef} type="file" accept="image/*" hidden onChange={handleImageUpload} />
                        <input ref={stampInputRef} type="file" accept="image/*" hidden onChange={handleImageUpload} />
                        <p style={{ fontSize: "0.72rem", color: "#444", textAlign: "center" }}>PNG with transparent background works best</p>
                      </div>
                    )}
                    ...
                    {currentSigDataUrl && (
                      <div>
                        <p className="panel-lbl" style={{ marginBottom: "8px" }}>Ready to Place</p>
                        <div className="sig-preview"><img src={currentSigDataUrl} alt="sig" /></div>
                        <div style={{ display: "flex", gap: "6px", marginTop: "8px" }}>
                          <button className="btn btn-ghost btn-sm" style={{ flex: 1 }} onClick={() => saveSigToStorage(currentSigDataUrl, signers.find(s => s.id === activeSigner)?.name || "Signature")}>
                            <Save size={12} /> Save
                          </button>
                          <button className="btn btn-ghost btn-sm" style={{ flex: 1 }} onClick={() => setCurrentSigDataUrl(null)}>
                            <X size={12} /> Clear
                          </button>
                        </div>
                        <div className="place-hint-box" style={{ marginTop: "10px" }}>
                          <strong>Click</strong> the PDF to place · <strong>Drag</strong> to move · <strong>🔵</strong> to rotate · <strong>⬛</strong> to resize
                        </div>
                      </div>
                    )}
                  </>
                ...
                {tab === "saved" && (
                  <>
                    ...
                    {savedSigs.length === 0 ? (
                      <p style={{ fontSize: "0.8rem", color: "#444", textAlign: "center", padding: "20px 0" }}>No saved signatures yet.<br />Create one and click Save.</p>
                    ) : (
                      <div className="saved-grid">
                        {savedSigs.map(s => (
                          <div key={s.id} className="saved-item" onClick={() => { setCurrentSigDataUrl(s.dataUrl); setTab("create"); }}>
                            <img src={s.dataUrl} alt="saved" />
                            <div className="saved-del" onClick={e => { e.stopPropagation(); deleteSavedSig(s.id); }}>
                ...
                {tab === "placed" && (
                  <>
                    ...
                    {placedSigs.length === 0 ? (
                      <p style={{ fontSize: "0.8rem", color: "#444", textAlign: "center", padding: "20px 0" }}>No signatures placed yet.</p>
                    ) : (
                      <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                        {placedSigs.map((sig, i) => {
                          const signer = signers.find(s => s.id === sig.signerId);
                          return (
                            <div key={sig.id} className={`placed-item${activeId === sig.id ? " active" : ""}`}
                              onClick={() => { setCurrentPage(sig.page); setActiveId(sig.id); }}>
                              <img src={sig.dataUrl} className="placed-thumb" alt="" />
                              <div className="placed-info">
                                <p className="placed-label">{signer?.name || "Signature"} #{i + 1}</p>
                                <p className="placed-sub">Page {sig.page + 1} · {Math.round(sig.rotation)}°</p>
                              </div>
                              <button className="placed-del" onClick={e => { e.stopPropagation(); deletePlaced(sig.id); }}>
                                <Trash2 size={11} />
                              </button>
                            </div>
                          );
            ...
            <div className="canvas-area">
              <div className="page-controls">
                <button className="ctrl-btn" disabled={currentPage === 0} onClick={() => setCurrentPage(p => p - 1)}>
                  <ChevronLeft size={14} />
                </button>
                <span>Page {currentPage + 1} / {totalPages}</span>
                <button className="ctrl-btn" disabled={currentPage === totalPages - 1} onClick={() => setCurrentPage(p => p + 1)}>
                  <ChevronRight size={14} />
                </button>
                <span className="zoom-sep" />
                <button className="ctrl-btn" onClick={() => setZoom(z => Math.max(0.3, z - 0.15))}><ZoomOut size={13} /></button>
                <span style={{ minWidth: "38px", textAlign: "center", fontSize: "0.75rem" }}>{Math.round(zoom * 100)}%</span>
                <button className="ctrl-btn" onClick={() => setZoom(z => Math.min(3, z + 0.15))}><ZoomIn size={13} /></button>
              </div>

              {pageImages[currentPage] && (
                <div ref={pageContainerRef} className="page-wrap"
                  style={{ width: `${Math.min(700, window.innerWidth - 40) * zoom}px`, cursor: currentSigDataUrl ? "crosshair" : "default" }}
                  onClick={currentSigDataUrl ? handlePageClick : undefined}
                  onMouseMove={onPageMouseMove} onMouseUp={onPageMouseUp} onMouseLeave={onPageMouseUp}>
                  <img src={pageImages[currentPage]} alt={`Page ${currentPage + 1}`} draggable={false} />

                  {currentPageSigs.map(sig => {
                    const signer = signers.find(s => s.id === sig.signerId);
                    const isActive = activeId === sig.id;
                    return (
                      <div key={sig.id} className={`sig-ov${isActive ? " active-ov" : ""}`}
                        style={{
                          left: `${sig.xPct}%`, top: `${sig.yPct}%`,
                          width: `${sig.wPct}%`, height: `${sig.hPct}%`,
                          transform: `rotate(${sig.rotation}deg)`,
                          borderColor: isActive ? signer?.color || "#F59E0B" : "transparent",
                        }}
                        ...
                        {isActive && (
                          <>
                            <div className="sig-del-btn" onClick={e => { e.stopPropagation(); deletePlaced(sig.id); }}><X size={10} /></div>
                            <div className="sig-resize" onMouseDown={e => { e.stopPropagation(); e.preventDefault(); setResizingId(sig.id); }} />
                            <div className="sig-rotate" onMouseDown={e => onRotateMouseDown(e, sig.id)}><RotateCw size={10} /></div>
                          </>
                        )}
                        {signer && (
                          <div className="sig-signer-badge" style={{ background: `${signer.color}22`, color: signer.color }}>{signer.name}</div>
                        )}
                      </div>
                    );
                  })}
                  ...
                  {currentSigDataUrl && currentPageSigs.length === 0 && (
                    <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", pointerEvents: "none" }}>
                      <div style={{ background: "rgba(245,158,11,0.07)", border: "1px dashed rgba(245,158,11,0.3)", borderRadius: "12px", padding: "16px 24px", color: "#a08040", fontSize: "0.85rem", fontStyle: "italic" }}>
